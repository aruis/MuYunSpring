...
class DynamicExportWebControllerTest {
    ...
    private DynamicExportWorkbookPlanBuilder planBuilder;
    ...
    void setUp() {
        ...
        planBuilder = mock(DynamicExportWorkbookPlanBuilder.class);
        writer = mock(ExcelWorkbookPlanWriter.class);
        mvc = MockMvcBuilders
                .standaloneSetup(new DynamicExportWebController(
                        recordService, activeTenantVerifier, planBuilder, writer))
    ...
    @Test
    void shouldDownloadTemplateWorkbook() throws Exception {
        ...
        when(planBuilder.build(descriptor)).thenReturn(plan);
        when(writer.writeToBytes(plan)).thenReturn(new byte[]{1, 2, 3});

        mvc.perform(post("/{moduleAlias}/export/template", MODULE))
                .andExpect(status().isOk())
                .andExpect(header().string("X-Export-FileName", "sales_order-template.xlsx"))
                .andExpect(header().string("Access-Control-Expose-Headers",
                        "Content-Disposition,X-Export-FileName"))
