...
class MenuWebControllerTest {
    ...
    void shouldExposeCurrentUserVisibleMenuTreeWithoutSchemeInput() throws Exception {
        ...
        MockMvc mvc = MockMvcBuilders.standaloneSetup(controller)
                ...
                .build();
        Menu root = menu("root-1", "scheme-1", "业务中心", null, MenuNodeType.GROUP);
        Menu child = menu("menu-1", "scheme-1", "客户", "crm.customer", MenuNodeType.ENTRY);
    ...
    void shouldExposeSchemeScopedMenuMaintenanceAndTree() throws Exception {
        ...
        Menu root = menu("root-1", "scheme-1", "业务中心", null, MenuNodeType.GROUP);
        Menu child = menu("menu-1", "scheme-1", "客户", "crm.customer", MenuNodeType.ENTRY);
        Menu inserted = menu("menu-2", "scheme-1", "订单", "crm.order", MenuNodeType.ENTRY);
        ...
        mvc.perform(post("/platform.menu-scheme/scheme-1/menus/insert")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"schemeId":"other-scheme","parentId":"root-1","title":"订单","nodeType":"entry","moduleAlias":"crm.order"}
                                """))
                .andExpect(status().isCreated())
    ...
    void shouldRejectCrossSchemeMenuUpdate() {
        ...
        MenuManagementWebController controller = new MenuManagementWebController();
        ReflectionTestUtils.setField(controller, "service", menuService);
        when(menuService.select("menu-1")).thenReturn(menu("menu-1", "other-scheme", "客户", "crm.customer", MenuNodeType.ENTRY));
    ...
    private Menu menu(String id, String schemeId, String title, String moduleAlias, MenuNodeType type) {
        Menu menu = new Menu();
        menu.setId(id);
