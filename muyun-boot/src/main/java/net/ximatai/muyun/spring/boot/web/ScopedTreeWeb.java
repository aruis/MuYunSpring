...
public interface ScopedTreeWeb<T extends EntityContract & TreeCapable, S extends TreeAbility<T>>
        extends TreeWeb<T, S> {
    String TREE_SCOPE_ATTRIBUTE = ScopedTreeWeb.class.getName() + ".treeScope";
    ...
    default void moveTree(HttpServletRequest request, String id, TreeSortWebRequest sortRequest) {
        ...
        if (scope.tenantId() == null) {
            service().moveInTree(scope.criteria(), id, sortRequest.previousId(), sortRequest.nextId(), sortRequest.parentId());
            return;
        }
