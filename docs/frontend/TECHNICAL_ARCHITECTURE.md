...
| 方向 | 当前状态 | 后续触发 |
| --- | --- | --- |
| Shell | 登录、token 保存、session/menu 启动、菜单页签、URL 恢复、mock client、后端模式构建已有测试或构建覆盖。 | 进入全局通知、租户切换、用户偏好或生产级登录安全时，再补真实 UI 出口和会话治理。 |
...
当前重点是先固定技术路线、分层骨架、团队协作模型、平台 shell 承载能力和可演进方向。后端页面交付接口已有阶段契约，但前端运行器尚未接入真实页面交付链路。
