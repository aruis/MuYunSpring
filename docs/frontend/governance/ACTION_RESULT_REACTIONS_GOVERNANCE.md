...
`web-contracts` 已允许统一 `WebActionResult` 承载 `message`、`resultType`、`changes`。`staticModuleClient` 会保留后端已经返回的动作事实，不在 normalize 时丢弃。后端统一 `ActionResult` / `DataChange` 输出需要在后续后端契约治理中补齐。
