...
| P2 | 页面交互 | 公式即时计算 | 建议作为页面运行态试算能力治理，不照搬 neo 独立接口。 | 待治理 |
| P2 | 配置与元数据 | 元数据视图、视图字段配置入口 | 服务层已有，需判断与 UI 配置是否重叠后再决定是否开放。 | 待治理 |
| P2 | 治理 | 模板复用管理入口 | 模板服务已有，但缺少稳定模板仓库和持久化管理边界。 | 待治理 |
| P2 | 运行态 | 标准批量删除动作贡献 | 不新增专用 URL；如吸收应走标准批量动作模型。 | 待治理 |
...
- 治理方向：先确认运行态 descriptor 是否依赖元数据视图作为稳定配置真相源；若依赖，再补轻量维护入口。
...
- 治理方向：作为页面运行态试算能力治理，不照搬 neo `/{moduleAlias}/formula/calculate` 形态；应明确不入库、使用已发布规则、输出变更字段和结构化诊断，并判断是否受 UI 配置字段范围约束。
...
- 治理结果：补齐 `platform.low_code_governance` 静态模块入口，开放配置包健康检查、发布、回滚、导出、导入 dry-run、导入草稿和草稿发布接口；模板复用因缺少稳定模板仓库暂不开放。
...
- 治理方向：先明确模板是否需要持久化、权限、版本和发布边界；边界稳定后再开放管理入口。
- 回流目标：`governance/OVERVIEW.md`、`governance/WEB_API.md`
...
- 治理方向：不新增 `/deleteBatch` 平行 URL；若吸收，应作为标准批量动作进入动作目录、权限和执行链路。
