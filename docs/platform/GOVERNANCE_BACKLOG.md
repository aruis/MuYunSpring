...
| P1 | 身份权限 | 角色授权批量矩阵操作 | 当前单点授权可用，批量授权体验和交接线索不足。 | 待治理 |
| P1 | 身份权限 | 角色菜单授权视图 | 当前有菜单剪枝和角色能力，缺少面向配置的角色菜单授权视图。 | 待治理 |
| P1 | 身份权限 | 用户选择器类查询接口 | 用户管理已有基础，接手和配置场景需要更清晰的选择器入口。 | 待治理 |
| P1 | 业务自动化 | 选中导出 | 当前导出主链路可用，缺少按选择集导出的显式入口。 | 待治理 |
| P1 | 业务自动化 | 用户导出模板 | 交换模板能力已有基础，用户级导出模板需要进一步抽象。 | 待治理 |
| P1 | 业务自动化 | 通用用户偏好 | 需要判断是纳入页面交互，还是作为横向用户偏好能力。 | 待治理 |
...
- 优先级：P1
- 当前判断：当前已有角色、动作授权、数据权限等基础能力，单点授权可用；批量矩阵更适合配置端交接和批量维护。
- 治理方向：补充按角色、模块、动作集合进行授权和撤销的接口，返回可回显的授权矩阵。
- 回流目标：`identity-permission/WEB_API.md`

#### 角色菜单授权视图

- 优先级：P1
- 当前判断：当前用户菜单和菜单剪枝能力已存在，但角色侧菜单授权视图不够明确。
- 治理方向：提供角色已授权菜单、可授权菜单树、保存角色菜单关系等配置入口。
- 回流目标：`identity-permission/WEB_API.md`

#### 用户选择器类查询接口

- 优先级：P1
- 当前判断：用户 CRUD 已有基础，但配置和授权场景通常需要按组织、角色、关键字快速选择用户。
- 治理方向：补充用户选择器查询，不替代用户管理 CRUD。
- 回流目标：`identity-permission/WEB_API.md`
...
- 优先级：P1
- 当前判断：导出主链路已有，但按选择集导出需要明确 URL 入口和参数约定。
- 治理方向：在数据交换专题下补充选中记录导出能力，复用现有导出执行链路。
- 回流目标：`business-automation/data-exchange/WEB_API.md`

#### 用户导出模板

- 优先级：P1
- 当前判断：交换模板能力已有基础，用户级导出模板需要确认与系统模板的继承、覆盖和权限边界。
- 治理方向：优先作为数据交换增强能力治理，不单独形成新的专题。
- 回流目标：`business-automation/data-exchange/OVERVIEW.md`

#### 通用用户偏好

- 优先级：P1
- 当前判断：可能服务于列表列宽、排序、筛选、导出模板等多个场景，需要先确定归属边界。
- 治理方向：若偏好可跨页面复用，宜作为横向平台能力；若只服务页面，则回到页面交互专题。
- 回流目标：`page/interaction/OVERVIEW.md` 或新增横向能力说明
