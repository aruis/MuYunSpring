...
代码声明或自动贡献的动作允许通过治理覆盖调整权限策略，包括访问方式、动作授权、数据授权、默认授予和权限动作继承；这些覆盖可收紧也可放宽，并可一键恢复声明默认值。执行器、动作编码、业务可用条件等执行事实始终由代码维护。最终生效策略由“声明事实 + 治理覆盖”合并后同时供 HTTP 授权、动态运行时动作和 OpenAPI 可见性使用。
