...
4. 创建业务硬换算规则。
   ...
   - 规则可按应用全局、模块或记录上下文生效；同一边优先使用更具体的上下文规则。
...
6. 发布模块定义。
   - 发布后 `FieldDefinition.measureUnit` 应包含单位分类、固定/可选模式、伴生字段、标准值字段、基准单位分类、基准单位、换算模式和单位必填约束。
...
| 缺口 | 建议 |
| --- | --- |
| 计量单位分类维护 | 提供应用内分类列表、启停、基准单位、维度和唯一性提示 |
| 单位目录维护 | 在分类下维护单位 code、标题、符号、精度、线性换算参数和排序 |
| 硬换算规则维护 | 提供 from/to 分类与单位选择、factor、scope、模块、上下文对象、有效期和启停 |
| 字段配置入口 | 在模块字段配置中提供计量单位开关、固定/可选单位模式、默认单位、伴生字段、标准值字段和换算模式 |
| 伴生字段生成提示 | 配置主数值字段时提示或自动准备 `quantityUnit`、`quantityBase`，并展示平台管理属性 |
| 发布前诊断 | 在发布或 dry-run 前展示缺少单位分类、缺少伴生字段、缺少标准值字段、上下文字段缺失等门禁 |
