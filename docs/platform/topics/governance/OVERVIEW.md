...
计量单位字段使用 `unitCategoryAlias` 时，模块包应在 dependency manifest 中声明 `MEASURE_UNIT` 依赖，依赖身份为 `applicationAlias + categoryAlias`。健康检查会对 metadata bundle 的字段清单做轻量契约检查：可选单位必须存在单位伴生字段，标准值字段必须独立存在，记录上下文字段如已配置也必须在包内字段清单中出现。
