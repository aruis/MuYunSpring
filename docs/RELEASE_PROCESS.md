...
```bash
./gradlew verifyAll publishReleaseToConsumerRepository
```

全部公共 artifact 会写入根目录 `build/consumer-repo`。外部应用将这个目录配置为 Maven repository
后，必须能够只凭坐标构建并启动；这验证 POM 的传递依赖和 Spring Boot 自动装配，而不依赖 Gradle
project dependency。

## Maven Central 发布

发布使用 `v<version>` Git tag 触发 `.github/workflows/release.yml`。版本号必须先写入根目录
`build.gradle.kts` 并合并到 `main`；workflow 只验证 tag 与该版本一致，再执行 `verifyAll` 和发布任务。
...
只有非 `-SNAPSHOT` 版本且 tag 与版本完全一致时，发布流程才会继续。发布任务为
`./gradlew publishReleaseToSonatype`。
