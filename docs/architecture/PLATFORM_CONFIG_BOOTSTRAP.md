...
裸库开发和演示启动可以通过 `muyun.demo-bootstrap.enabled=true` 显式创建一组租户、组织、部门、职员、租户管理员账号和租户管理员账号角色授权样例数据。该能力属于可选演示 bootstrap，不进入 `InitialDataAbility` 平台 baseline。演示租户管理员必须作为租户内普通用户经账号角色授权获得权限，不能复用系统超级管理员的 system user 短路。
