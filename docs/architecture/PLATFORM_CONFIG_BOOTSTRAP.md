...
平台字段类型配置表达一类字段的默认事实，例如运行态基础类型、默认长度/精度和默认查询操作符。字段配置表达某个字段的覆盖，例如字段级长度/精度、字典绑定、是否可查询和查询操作符。字段配置未声明查询时继承字段类型默认；只有显式关闭时才禁用查询。字段引用配置独立表达“源字段 -> 目标元数据”的引用关系，以及标题输出和投影输出；当前标题/投影展示只支持当前发布模块内的目标实体，跨模块展示后续由跨模块运行态 resolver 单独建设。发布时由“字段结构 + 字段类型 + 字段配置”编译成运行态 `FieldDefinition`，由字段引用配置编译成运行态 `EntityReferenceDefinition`。
