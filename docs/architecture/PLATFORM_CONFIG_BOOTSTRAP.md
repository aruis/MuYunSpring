...
`PHYSICAL`、`COMPANION` 和 `SHADOW` 字段会编译为动态物理字段并参与 schema 映射；`VIRTUAL` 字段只进入运行态字段契约，不参与建表和 DDL。虚拟字段不能声明必填、唯一、索引、排序、标题、可查询、默认值或校验正则，也不能使用存储保护、计量单位和金额存储行为。复杂派生值如何计算、何时注入记录、是否允许参与页面展示和导入校验，由后续派生值计算入口单独定义。
