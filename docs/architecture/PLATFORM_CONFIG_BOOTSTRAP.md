...
应用是平台配置的顶层归属。模块、元数据、菜单和字典都应能归属到应用。应用不是树，但需要支持排序，便于管理端按业务顺序展示。
...
| `moduleType` | `STANDARD` / `CUSTOMIZED` / `BUILT_IN_PAGE` / `REPORT` / `DASHBOARD` |
| `enabled` | 是否启用 |
| `sortOrder` | 排序 |

模块 alias 创建后不允许手动修改。模块按“应用 -> 多级模块”组织成树，排序范围限定在同一应用和同一父模块下。`moduleType` 表达模块入口/页面形态，不表达静态或动态实现方式。静态模块可以没有元数据；动态模块必须有一个主元数据。
