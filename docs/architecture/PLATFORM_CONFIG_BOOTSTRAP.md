...
字段配置支持两层口径：不带 `relationId` 的默认配置作用于元数据字段本身；带 `relationId` 的配置只作用于某个模块元数据关系，用于同一元数据在不同模块入口下呈现不同查询、字典或引用展示行为。结构字段、物理列名、字段类型以及长度/精度等物理形态仍归属元数据字段和默认字段配置，关系级覆盖不能改写物理形态。发布时按“字段类型默认 -> 字段默认配置 -> 关系级配置”合成运行态 `FieldDefinition`。

字段引用配置独立表达“源字段 -> 目标元数据”的引用关系，以及标题输出和投影输出；同样支持默认配置和关系级覆盖。当前标题/投影展示只支持当前发布模块内的目标实体，跨模块展示后续由跨模块运行态 resolver 单独建设。发布时由字段引用配置编译成运行态 `EntityReferenceDefinition`。运行态 descriptor 输出模块、实体、字段、关系、引用、实体标准动作和基础视图；动作只表达标准能力入口，不绑定 UI、路由或权限策略。基础视图只表达列表/表单字段顺序、显示状态、标题和控件类型，不承载复杂布局。
