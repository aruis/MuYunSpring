...
字段引用配置独立表达“源字段 -> 目标元数据”的引用关系，以及标题输出和投影输出；同样支持默认配置和关系级覆盖。当前标题/投影展示只支持当前发布模块内的目标实体，跨模块展示后续由跨模块运行态 resolver 单独建设。发布时由字段引用配置编译成运行态 `EntityReferenceDefinition`。运行态 descriptor 输出模块、实体、字段、关系、引用、动作和基础视图；动作配置挂在模块元数据关系上，可覆盖标准动作的启停、标题、级别和权限挂点，也可声明自定义动作。模块级动作是主元数据动作的模块级投影；子元数据动作默认只属于关系内部，是否提升为模块级动作后续由明确业务场景驱动。基础视图只表达列表/表单字段顺序、显示状态、标题和控件类型，不承载复杂布局。
