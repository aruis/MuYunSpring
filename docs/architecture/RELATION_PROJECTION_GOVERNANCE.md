...
2. 字段输出保护必须经过 SQL Map 输出执行器；输出脱敏字段可走 SQL 投影，加密和签名等存储保护字段继续回退实体查询。
3. `postReadTransforms` 只允许已被 SQL 输出路径支持的转换；不认识的转换继续回退实体查询。
...
4. SQL 路径返回 Map 时已支持输出脱敏字段保护；涉及加密、签名或其他需要存储态伴生字段的保护场景仍回退实体查询，直到 SQL 投影能显式选择伴生字段并复用解密/验签链路。
