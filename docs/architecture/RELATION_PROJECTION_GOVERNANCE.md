...
当前 SQL plan 已携带 `ProjectionGraph`。静态引用、动态 `ONE` 引用投影和 post-read transform 会先进入 graph；`1:1 / N:1` reference projection 的 SQL select/join 已由 graph edge/node 驱动。legacy relation join、字段保护和字典标题执行仍保留现有适配路径，后续再继续收敛为 graph transform 执行。
