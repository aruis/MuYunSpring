...
- **多实例并发首次启动会失败。** 当前版本假设单实例启动：两个实例同时冷启动跑同一 migration 时，`MigrationVersionStore.recordVersion` 的 insert 会撞 partial unique index 抛 duplicate key，**后启动的实例启动失败**。生产部署应保证单实例启动（蓝绿切换、滚动升级时等迁移完成再起下一个实例），或等待后续版本提供 upsert / 分布式锁兜底（见 TD-020）。
