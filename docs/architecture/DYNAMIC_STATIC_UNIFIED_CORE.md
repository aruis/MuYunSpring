...
低代码动态表单阶段的“动静一体”不表示要同时建设一套静态表单 UI。M8 以后聚焦的是动态表单页面交付；动静一体约束的是动态表单依赖的平台底层能力仍要与静态模型共享同一套数据访问、能力语义、生命周期、权限、审计、租户和事务边界，不能为了页面配置另起一套只服务动态表单的基础内核。
...
UI 配置、查询模板、菜单入口、动态页面 bootstrap、引用候选、附件页面交付等低代码表单能力属于动态模块交付面，不要求同步建设静态表单 UI。但这些能力调用的数据读写、动作权限、数据权限、审计、附件业务关系、事务和生命周期仍必须回到平台统一能力链路，不能绕过静态/动态共用的底层契约。
