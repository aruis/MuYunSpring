...
| TD-020 | 数据迁移多实例并发首次启动会失败 | `MigrationVersionStore.recordVersion` 的 insert 在 partial unique index 兜底下，并发启动时第二个实例会撞 duplicate key 抛异常导致启动失败。当前假设单实例启动（蓝绿 / 滚动升级时等迁移完成再起下一个实例） | 进入多实例并发启动场景时，将 recordVersion 改为 upsert 语义（`INSERT ... ON CONFLICT DO UPDATE`），或在 `MigrationBootstrap` 启动时加分布式锁 |
| TD-021 | 数据迁移运行态可观测性弱 | 启动时只有日志输出步骤号，没有进度端点、失败告警、dry-run 预演；长时间迁移无法外部观测；生产部署前无法预演 | 进入复杂迁移治理场景时，提供 `MigrationProgress` 端点（当前 alias + 已应用版本 + 剩余步骤）、dry-run 模式、失败告警集成 |
| TD-022 | 数据迁移不支持反向迁移（回滚） | 版本表只向前追加；从高版本回滚到低版本时，`currentVersion > finalVersion` 让所有步骤早退跳过，但已改的数据留在新状态，代码版本与数据状态不一致 | 进入需要精细回滚治理的场景时，为 `AbstractMigration` 增加 `rollback()` 钩子；或维持"数据迁移版本不可回滚"的口径，通过部署流程保证（避免跨数据迁移版本回滚） |
