...
| 编号 | 问题 | 风险 | 回收条件 |
| --- | --- | --- | --- |
| TD-039 | 管理工作台的滚动容器策略尚未形成统一布局契约 | `RecordDetailPanel` 已支持固定头部与内容区滚动，但组合层此前未将该布局意图统一暴露；业务页容易在整页滚动、局部滚动和自行添加 `overflow` 之间产生不一致，进而出现双滚动条、固定操作栏失效或窄屏退化 | 出现第二个内容较长的管理工作台，或统一治理 Workspace Tab 布局时，定义流式与受限高度两种显式策略；由 `StaticManagementLayout` / 工作台宿主统一持有滚动容器，业务页仅声明布局意图，并补长表单单滚动容器的组件或视觉契约测试 |
