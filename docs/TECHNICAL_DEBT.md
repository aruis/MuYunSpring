...
| TD-010 | 静态仓储入口仍需证明所有模型建表都复用平台字段和索引归一化 | 单个静态业务已验证租户内唯一迁移，但任意静态模型的 `@Column(unique=true)` 是否都经由统一 mapper 收口还缺少 contract 证据 | 增加通用静态模型仓储 contract，覆盖自动建表、平台字段、租户唯一归一化和旧唯一索引替换 |
