...
| DD-004 | 平台错误响应的真实 UI 出口和特殊业务链路尚未接入 | 后端统一错误 envelope、traceId、基础错误码、前端 `AppError`、全局展示槽位判定、登录特殊链路和静态业务失败 `ActionMessage` 基础契约已落地；但当前前端尚未进入真实动态表单、查重、导入面板或工作流动作区开发，也没有实际 toast/modal/page-error UI adapter。动态表单保存、动态 local edit、页面 bootstrap 和运行态配置解析中的失败语义暂不在当前阶段继续迁移，避免在缺少真实 UI 出口前提前固化错误展示和定位契约 | 进入 shell 全局通知、动态表单保存、查重确认、导入行级错误、工作流动作反馈或 online 子应用错误治理时，按 `docs/frontend/governance/ERROR_HANDLING_GOVERNANCE.md` 接入对应 UI 出口和特殊 handler；正式推进动态表单链路时，再统一评估动态运行态异常、字段/子表定位、`ActionMessage` code、HTTP 状态和前端局部展示契约 |
...
| TD-029 | 关联投影查询仍需继续平台化 | 当前已将静态列表 SQL join 上移到来源无关的 `RelationProjectionReadService` / `RelationProjection*` 能力，并收敛输出字段、cardinality、递归深度、join 数量和 relation 字段查询边界；动态主实体 `ONE` 引用投影已有最小 adapter，动态列表 `uiConfigId` 查询已能复用同一 SQL 投影门面，受保护字段会回退实体查询，但字段保护 SQL 输出执行器、projection plan 缓存、模块关系、子实体引用、`MANY` 聚合和字典标题尚未完成 | 按 [关联投影查询治理](architecture/RELATION_PROJECTION_GOVERNANCE.md) 分阶段回收，动态表单业务推进时继续把动态引用字段、模块关系和字典标题编译到同一套 `RelationProjection` |
...
| TD-033 | 前端标准删除和启停动作尚未携带客户端 expected version | Ability 层已经支持带实体删除和更新的乐观锁，静态编辑保存也要求保留 `version`；但当前前端标准 `delete/enable/disable` CRUD 客户端只按 id 调接口，后端会按当前库内版本执行动作，无法识别用户基于旧详情触发删除或启停的冲突 | 进入删除/启停并发治理或记录动作 payload 统一治理时，扩展标准 Web CRUD 动作请求，允许携带 `version`/action context，并让后端按客户端 expected version 执行条件写入 |
