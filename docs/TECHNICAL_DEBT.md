...
| DD-005 | Web malformed 请求的统一错误 envelope 尚未定稿 | CORS 已收敛为默认关闭，仅 `local` profile 显式开放本机前端来源；但登录请求缺 body、字段类型异常、空参数等 malformed 场景尚未纳入统一错误 envelope | 进入平台错误响应治理或对应真实页面交付时，统一 malformed 请求的错误码、HTTP 状态、字段定位与前端展示边界 |
