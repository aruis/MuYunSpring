...
| TD-011 | 静态/动态写入尚未形成统一乐观锁 CAS contract | `version` 当前主要承担生命周期递增，旧版本并发写入失败语义不稳定 | 在数据入口补齐 expected version 更新/删除接口，并用真实数据库测试锁住冲突失败行为 |
...
| TD-013 | 复杂 Criteria 真实库组合覆盖不足 | 单项查询能力已验证，但复杂条件、分页、排序和非法字段拒绝组合可能在真实 SQL 层暴露问题 | 扩展真实数据库 contract，覆盖 `IN/NOT_IN/range/null/raw + sort + page` 和非法字段拒绝 |
