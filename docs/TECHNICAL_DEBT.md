# 技术债记录

本文只记录已经确认但当前目标暂缓处理的平台级技术债。执行过程、临时复核和普通 TODO 不放在这里。

## 分层结论

基础能力没有必须阻塞平台配置业务的技术债。平台配置业务的重点不是继续打磨所有基础设施，而是验证能力组合是否足够省代码、够直观。

下列债务按回收触发点分层管理。未到触发点前，不作为当前开发阻塞；一旦进入对应业务场景，应优先回收。

## 配置自举时关注

| 编号 | 问题 | 风险 | 回收条件 |
| --- | --- | --- | --- |
| TD-008 | 租户切换和系统态授权审计尚未最终封口 | 登录态已通过 Bearer token、`CurrentUserContext` 和 `TenantContext` 接入身份体系；但用户多租户切换、后台任务系统态授权审计，以及无上下文请求是否允许进入系统态仍需要更严格治理 | 进入多租户账号切换、后台任务审计或生产安全加固时，统一租户切换协议和系统态授权记录 |
| TD-009 | 租户内唯一约束与软删值复用策略尚未最终定稿 | 当前唯一索引默认约束 `(tenant_id, field)`，同租户软删后仍不能复用唯一值；如果未来要支持 active-only unique，需要调整索引和查询策略 | 应用、模块、元数据、字典 alias 唯一约束先沿用不可复用口径；后续结合审计、回收站和数据恢复口径定稿 |

## 平台执行底座前关注

| 编号 | 问题 | 风险 | 回收条件 |
| --- | --- | --- | --- |
| TD-012 | 动态 schema 演进仍缺少多实体 DDL 失败补偿 | 当前已覆盖新增字段、字段删除 dry-run、非加法 strict 拒绝、失败不污染运行态，以及失败异常携带已完成实体迁移结果；但数据库已执行部分 DDL 后仍没有统一补偿或恢复策略 | 进入配置发布中心或批量发布治理时，设计 schema 发布批次、执行日志、失败恢复和必要的人工补偿协议 |
| TD-013 | 复杂派生字段读取注入和导入参与范围尚未定稿 | 默认值、校验正则、可复制、写保护、范围查询和虚拟字段基础形态已经补齐到字段配置链、schema 映射、页面查询契约、表单保存边界、公式试算入口和已注入虚拟值展示输出；虚拟字段不会进入动态 SQL 查询、排序、quickSearch 或导出查询；复杂派生行为如果过早进入列表/详情读取自动计算或导入校验，会扩大运行态心智负担 | 进入公式虚拟字段列表/详情自动计算、导入校验派生字段或跨记录聚合派生时，先定义注入时机、失败语义、性能边界和可参与场景，再接入字段配置链 |
| TD-018 | 业务时区来源尚未模型化到租户、组织和用户主数据 | 当前只提供平台默认时区配置、显式上下文时区和 resolver 扩展点；生产 Spring 路径已复用 `PlatformTimeService`，但尚未定义租户/组织/用户时区字段、优先级配置入口和治理页面 | 进入跨区域组织、用户本地化展示或租户级时区治理时，补齐主数据字段、解析优先级、配置入口和迁移策略 |
| TD-019 | 非 Spring 手工构造路径不承诺读取 boot 默认时区配置 | 兼容构造器和测试便利构造仍可能 `new PlatformTimeService()` 或按传入 `Clock` 回退；生产 Spring 路径已有契约测试锁住默认时区贯穿动态查询、编码规则和自然日历 | 如果要把平台默认时区提升为所有构造方式的强契约，收紧兼容构造器或提供统一测试工厂，并回收直接 `new PlatformTimeService()` 的生产用法 |

## 待讨论决策

| 编号 | 议题 | 当前口径 | 触发条件 |
| --- | --- | --- | --- |
| DD-001 | 系统基础字段是否需要进入可配置字段清单 | 先不把 id、tenantId、deletedAt、createdAt、updatedAt 等基础字段开放为普通元数据字段 | 当管理端需要展示、筛选或审计这些字段时，再决定是只读系统字段还是可配置字段 |
| DD-002 | 同一元数据在不同模块下是否允许覆盖更多行为 | 当前只允许关系级覆盖查询、字典和引用展示等运行行为，不允许覆盖物理字段形态 | 出现“同一物理表在不同入口下有明显不同展示/校验策略”的业务后，再扩展覆盖范围 |
| DD-003 | 元数据和字段删除时如何治理运行态引用关系 | 当前配置保存即刷新覆盖新增和更新；删除/软删被模块关系、模块字段、视图、动作等引用的配置时，尚未决定是禁止删除、级联软删还是进入失效治理流程 | 进入配置删除、配置回收站或配置包差异删除能力时，先定引用治理策略，再补运行态刷新契约 |
| DD-004 | 平台错误响应的真实 UI 出口和特殊业务链路尚未接入 | 后端统一错误 envelope、traceId、基础错误码、前端 `AppError`、全局展示槽位判定和登录特殊链路已落地；但当前前端尚未进入真实动态表单、查重、导入面板或工作流动作区开发，也没有实际 toast/modal/page-error UI adapter | 进入 shell 全局通知、动态表单保存、查重确认、导入行级错误、工作流动作反馈或 online 子应用错误治理时，按 `docs/frontend/governance/ERROR_HANDLING_GOVERNANCE.md` 接入对应 UI 出口和特殊 handler |
| DD-005 | 平台 Web 安全默认值和本地联调默认值边界尚未定稿 | 当前后端 CORS 和前端真实链路联调已能支撑本机开发，但平台 Web 默认策略应逐步从“方便联调”收敛到“默认关闭、按应用或 profile 显式开启”；登录请求缺 body、字段类型异常、空参数等 malformed 场景也尚未纳入统一错误 envelope | 进入生产部署 profile、跨域安全配置或平台错误响应治理时，统一 CORS 默认策略、联调 profile 和 malformed 请求错误语义 |
| DD-006 | 平台超级管理员是否应免业务动作授权尚未定稿 | 当前 `platform.user.super_admin` 通过初始化数据绑定 `platform.role.super_admin`，再按已注册模块动作逐条生成 `RoleAction`，运行时仍走普通角色授权链路；如果未来改成超级管理员免业务动作授权，可以减少自举授权补齐和新动作漏授权风险，但需要明确它只豁免业务权限，不绕过登录、租户态、数据安全、危险运维动作和审计记录 | 进入生产安全模型收口、权限初始化简化、超级管理员运维治理或模块动作大规模扩展时，统一定义 super admin bypass 的作用域、审计标记、租户边界和与普通角色授权的优先级 |

## 场景触发回收

| 编号 | 问题 | 风险 | 回收条件 |
| --- | --- | --- | --- |
| TD-002 | 聚合装配出的 child 已明确按 RAW 查询读取，并有契约测试锁住不执行 child service 完整 `afterSelect`；HYDRATED 聚合读取暂未开放 | 单独读取子实体和父聚合带出子实体的语义仍需保持使用者可预期 | 出现需要嵌套聚合或深度加载的业务场景时，设计显式 HYDRATED 入口 |
| TD-010 | 引用能力的标题、选项和字段投影已明确按 RAW 读取目标记录，并有契约测试锁住不触发目标 service `afterSelect`；更通用的加载模式暂未设计 | 复杂聚合读取时，引用、父子关系和缓存可能需要更明确的加载深度策略 | 出现跨层级 HYDRATED 引用读取需求时，再设计统一加载模式 |
| TD-015 | 职员任岗主岗唯一仍缺少强并发保护 | 已提供事务性主岗切换能力，常规请求会先降级同职员其他启用主岗再设置目标主岗；但没有条件唯一索引或显式锁时，强并发写入仍可能绕过应用层预查 | 进入组织人事并发导入或平台级条件唯一约束建设时，补条件唯一索引或显式锁 |
| TD-016 | 职员主账号唯一仍缺少强并发保护 | 已提供事务性主账号切换能力，常规请求会先降级同职员其他启用主账号再设置目标主账号；但没有条件唯一索引或显式锁时，强并发写入仍可能绕过应用层预查 | 进入账号开通批处理、多端账号治理或平台级条件唯一约束建设时，补条件唯一索引或显式锁 |
| TD-017 | 外部写入、后台任务和异步批次尚未建设专题流水 | 平台运行审计只记录动作入口和必要身份上下文，不能解释幂等、重试、回执、批次进度和失败恢复 | 进入外部系统接入、统一后台任务调度或异步导入执行时，按 [审计与专题流水边界](platform/AUDIT_AND_PROCESS_LOG_BOUNDARY.md)、[外部写入接入边界](platform/EXTERNAL_WRITE_BOUNDARY.md) 和 [后台任务与异步批次边界](platform/BACKGROUND_JOB_AND_BATCH_BOUNDARY.md) 建设对应专题流水 |
| TD-020 | 工作日历和 SLA 仍停留在自然时间边界 | `BusinessCalendarService` 当前默认实现只表达自然 elapsed time，workflow 超期仍按节点激活时刻加自然分钟数计算，不跳过节假日或非工作时间；`BusinessCalendar.workingTimeAware` 只能区分自然日历和未来工作日历能力，不是完整日历类型模型 | 进入审批 SLA、工单 SLA 或营业时间承诺时，补 calendar type/能力枚举、工作时段、节假日、时区归属和 SLA 计算策略，再接入 workflow 节点定义 |
| TD-021 | 登录 session 生命周期审计和清理策略尚未完整模型化 | 当前 session 已持久化到 `iam_user_session`，支持 token hash、滑动过期、绝对过期、多端登录、登出和用户失效撤销；但过期 session 只在访问时拒绝，不立即写入撤销状态，也没有后台清理、保留周期和审计汇总策略 | 进入登录安全审计、运维会话管理、长期运行数据清理或账号安全治理时，设计 session 过期落库、清理任务、保留周期和审计查询 |
| TD-022 | 按用户撤销 session 仍是查询后逐条更新 | 当前 `revokeUserSessions` 为保证语义正确，按用户查询 session 后逐条乐观更新；用户多端数量较少时足够，但批量密码重置、组织禁用或账号安全事件会放大数据库往返 | 进入账号安全批处理、批量禁用或平台 DAO 条件批量更新能力建设时，补 `userId + revokedAt is null` 的条件批量撤销能力 |
| TD-023 | 模块锚定的运行态数据校准能力尚未建设 | 初始化数据能力只治理平台托管事实，不承接已上线业务记录的历史值转换、字段搬运、批量归一或跨版本数据修补；如果提前引入任意 alias 的启动迁移脚本，会绕开 `moduleAlias`、运行态 descriptor、租户、审计和后台任务边界 | 出现真实历史业务数据修补、跨版本记录校准或动态模块运行态数据迁移需求时，以 `moduleAlias` 为锚点设计运行态门禁、dry-run、执行报告、多租户作用域、失败恢复和审计/后台任务接入 |
| TD-024 | 前端系统时间展示格式尚未平台化 | 后端返回 ISO UTC 时间字符串是合理 API 契约，但当前前端系统信息区等页面直接展示原始字符串，缺少统一 DateTime 展示组件、格式规范、本地时区换算和非法值兜底 | 进入前端系统信息区统一治理、审计时间展示、用户本地化展示或前端平台组件规范收口时，沉淀统一时间展示能力，并与业务时区模型债务 `TD-018` 保持边界一致 |
| TD-025 | 后端业务异常提示尚未形成可展示中文口径 | 当前部分业务拒绝原因仍直接抛出英文技术文案，例如岗位分类被岗位引用时返回 `position category is referenced by positions`；前端全局提示会忠实展示后端 message，不在页面侧翻译业务异常 | 进入统一错误码、业务异常本地化、管理端全局提示治理或生产可用性收口时，为业务异常补稳定错误码、中文默认文案和必要参数，明确前端展示 message 与 traceId 的边界 |
| TD-026 | 应用归属引用保护仍缺少贡献式检查契约 | 当前删除应用时只在 `ApplicationService` 中显式检查模块、元数据和字典类目等已接入引用；仓库内其他 `applicationAlias` 归属模型后续增多时，如果继续扩硬编码清单，容易遗漏并产生孤儿配置数据 | 进入计量单位、工作流、配置包或更多应用归属模型的删除治理时，沉淀应用归属引用检查贡献接口或注册表，由各业务能力贡献引用存在性、展示名称和错误详情，`ApplicationService` 只编排统一拒绝语义 |
| TD-028 | `QuerySchema` 外部查询值描述仍过于粗糙 | `QuerySchema.ExternalCriteria` 目前只暴露 key，并固定为 `OBJECT` / `PAGE_CONTEXT`，缺少字段结构、来源语义和校验契约；随着静态查询模板、动态页面上下文和外部查询值增多，ability 层可能继续携带页面交付语义 | 后续扩展 external query values 前，引入 `ExternalCriteriaDescriptor` 或等价能力契约，由能力层声明 valueType、来源、对象字段结构和校验边界，Web 层只负责序列化 |
| TD-029 | 静态查询描述仍依赖字段名约定推断 | `QueryDescriptors.simple` 目前用字段名集合推断 valueType、操作符、标题、quickSearch 和可排序字段，能快速回收旧 Web 查询适配器并改善静态 schema 展示，但业务字段名进入 ability 核心会带来同名不同义、字段漏判和静态/动态查询契约分叉风险 | 静态查询字段继续扩展、出现更多非通用字段类型或查询配置需要对外稳定时，沉淀静态模型 QueryDescriptor 编译器，从 Java 字段类型、模型契约或注解生成查询字段；保留 `simple` 为保守基线或测试便利 |
| TD-030 | 静态查询字段尚未自动合并 `@OptionField` 元数据 | 当前静态 service 需要在 `QueryDescriptor` 中手动把同名 `QueryField` 与模型上的 `@OptionField` 绑定；单字段接入成本可接受，但多个静态模型接入字典或枚举后会重复声明 option binding、selection mode 和 title 输出字段 | 出现第二个以上静态模型需要选项字段查询 schema 时，沉淀静态查询字段编译/合并能力，从 Java 字段注解自动补齐同名 `QueryField` 的选项元数据，业务 service 只保留查询语义 |

## 运维治理触发回收

| 编号 | 问题 | 风险 | 回收条件 |
| --- | --- | --- | --- |
| TD-005 | 引用依赖缓存失效已有本地进程内索引和动静态路径闭环，但还不是跨节点治理能力 | 多实例部署和复杂批量刷新仍需要更完整的依赖生命周期管理 | 进入缓存治理或动态发布中心时，补跨节点事件和批量重建策略 |
| TD-006 | `CacheAbility` 仍使用进程内 Caffeine 本地缓存 | 当前已有默认容量、全量列表 TTL 和 runtime namespace 清理，但还不是可观测、可替换的缓存管理器 | 当缓存需要监控、跨节点一致性或业务级策略时，引入运行态缓存管理器 |
