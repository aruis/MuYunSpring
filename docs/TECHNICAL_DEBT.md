...
| DD-004 | 平台错误响应如何驱动前端通知和恢复行为 | 当前已区分认证缺失、认证失败、权限不足和平台配置缺失等后端错误语义，前端只在 401 场景做 session 恢复；但不同错误 code/status 应映射为 toast、modal、页面错误、表单字段错误、登录恢复或静默处理，尚未形成统一前端行为协议 | 进入全局通知、表单保存错误、动作执行反馈或平台前端错误治理时，定义错误 envelope、展示级别、交互恢复策略和可覆盖规则 |
