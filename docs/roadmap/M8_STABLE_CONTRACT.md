...
| 列表查询 | `POST /{moduleAlias}/query` | 支持 `uiConfigId` 列投影、`queryTemplateId + externalQueryValues`、任意嵌套 `criteria`、兼容扁平 `conditions`、`quickSearch + quickSearchFields`、分页排序和可选导航会话。 |
...
5. 运行时总合并规则为 `queryTemplateId/externalQueryValues AND conditions AND criteria AND quickSearch`；引用候选、汇总和动态导出复用同一语义。
