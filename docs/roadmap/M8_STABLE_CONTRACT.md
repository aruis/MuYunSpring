...
3. 列表查询、汇总面板、动态导出和引用候选复用同一查询模板编译链路，额外手工条件只作为运行态 Criteria 叠加，不生成第二套查询协议。运行态 `criteria` 支持任意层级 `AND/OR` 分组嵌套；旧 `conditions` 保留为顶层扁平 `AND` 兼容入口。快速查询只在已发布 LIST 配置的可见主关系文本字段范围内编译为 `LIKE` 条件；显式指定字段时，字段必须在该范围内。
...
| 列表查询 | `POST /{moduleAlias}/query` | 支持 `uiConfigId` 列投影、`queryTemplateId + externalQueryValues`、`queryForm`、任意嵌套 `criteria`、兼容扁平 `conditions`、`quickSearch + quickSearchFields`、分页排序和可选导航会话。 |
| 汇总面板 | `POST /{moduleAlias}/query/summary` | 使用与列表一致的查询上下文，从已发布 LIST 配置的 `layoutJson.summaryPanel.items` 读取汇总项。 |
| 引用候选 | `POST /{moduleAlias}/references/{fieldName}/resolve` | `sourceUiConfigId` 校验来源 FORM/DETAIL 上下文，`uiConfigId` 校验目标 LIST/REFERENCE 上下文，`queryTemplateId` 可覆盖字段默认候选模板，并复用列表 `criteria` 复杂查询树。 |
...
`criteria` 复杂查询树使用以下稳定语义：

1. 每个分组节点包含 `operator`、`conditions` 和 `groups`；`operator` 只允许 `AND` 或 `OR`，缺省按 `AND` 处理。
2. 同一分组内的直接 `conditions` 和子 `groups` 按当前分组 `operator` 连接；子分组递归适用同一规则，不限制嵌套层级。
3. 单个 `condition` 仍使用 `fieldName + operator + values`，字段和操作符必须通过动态字段查询契约校验。
4. 请求根上的旧 `conditions` 不表达嵌套语义，只作为顶层扁平 `AND` 兼容入口；新前端复杂查询应优先使用 `criteria`。
5. `queryForm` 用于前端按已发布 LIST UI 的可见主表字段名提交表单值，平台按字段默认查询操作符编译；空值会被忽略，未出现在该 LIST 配置中的字段会被拒绝。
6. 运行时总合并规则为 `queryTemplateId/externalQueryValues AND queryForm AND conditions AND criteria AND quickSearch`；列表、汇总和动态导出复用同一语义，引用候选复用查询方案和 `criteria` 复杂查询树。
