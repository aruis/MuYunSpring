...
} from '../src/platform-components/softDeletedConflictHandler.ts';
import type { UiConfirmOptions } from '../src/vue-ui-antdv/index.ts';

test('extractSoftDeletedConflict extracts info from matching error', () => {
...
test('extractSoftDeletedConflict returns undefined for non-matching error code', () => {
...
test('extractSoftDeletedConflict returns undefined for plain error', () => {
...
test('extractSoftDeletedConflict handles missing optional fields', () => {
...
test('useSoftDeletedConflictHandler returns false for non-matching error', async () => {
...
test('useSoftDeletedConflictHandler shows modal and returns true for matching error', async () => {
...
test('useSoftDeletedConflictHandler calls onNavigateToRecycleBin when confirmed', async () => {
...
test('useSoftDeletedConflictHandler does not navigate when cancelled', async () => {
  let navigated = false;
  const handler = useSoftDeletedConflictHandler({
...
test('createSoftDeletedConflictErrorHandler creates handler with correct code', () => {
...
test('createSoftDeletedConflictErrorHandler handle returns true for matching error', () => {
  const handler = createSoftDeletedConflictErrorHandler({
    confirmAction: async () => true,
  });
