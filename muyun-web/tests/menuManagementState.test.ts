...
test('menu management keeps entry fields only when module alias is bound', () => {
...
test('menu management validates open mode only for bound module entry', () => {
...
test('menu management cancel restores selected parent after child creation', () => {
  const schemeContext = createFakeContext<MenuScheme>('platform.menu_scheme');
  const menuContext = createFakeContext<MenuRecord>('platform.menu-scheme/scheme-1/menus');
  const state = createMenuManagementState(
    schemeContext,
...
test('menu management cancel restores selected menu after root creation', () => {
  const schemeContext = createFakeContext<MenuScheme>('platform.menu_scheme');
  const menuContext = createFakeContext<MenuRecord>('platform.menu-scheme/scheme-1/menus');
  const state = createMenuManagementState(
    schemeContext,
...
test('menu management keeps create draft when menus reload', () => {
  const schemeContext = createFakeContext<MenuScheme>('platform.menu_scheme');
  const menuContext = createFakeContext<MenuRecord>('platform.menu-scheme/scheme-1/menus');
  const state = createMenuManagementState(
    schemeContext,
