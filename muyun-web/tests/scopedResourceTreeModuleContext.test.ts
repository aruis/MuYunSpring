...
import { createScopedResourceTreeModuleContext } from '../src/platform-components/scopedResourceTreeModuleContext.ts';
...
test('scoped resource tree context targets nested menu resource paths', async () => {
...
test('scoped resource tree context returns empty tree before resource path is available', async () => {
  const context = createFakeModuleContext();
  const scoped = createScopedResourceTreeModuleContext<MenuRecord, MenuRecord>(context, {
