...
import {
  ...
  positionTitleOf,
} from '../src/views/positionManagementState.ts';

test('position management state creates category-bound position drafts', () => {
...
test('position management state trims position drafts and requires category', () => {
...
test('position management state matches positions only inside selected category', () => {
  assert.equal(
...
test('position management state trims category drafts and clears blank parent id', () => {
...
test('position management state exposes stable fallback titles', () => {
...
test('position management state loads positions inside selected category', async () => {
...
test('position management state saves normalized positions with required category', async () => {
  const calls: unknown[] = [];
  const categoryContext = createContext<PositionCategory>('iam.position_category');
  const positionContext = createContext<Position>('iam.position', {
    insert: async (record) => {
...
test('position management state respects delete confirmation result', async () => {
  const calls: string[] = [];
  const categoryContext = createContext<PositionCategory>('iam.position_category');
  const positionContext = createContext<Position>('iam.position', {
    delete: async (id) => {
...
test('position management state rejects position actions without contributed permissions', async () => {
  const calls: string[] = [];
  const categoryContext = createContext<PositionCategory>(
    'iam.position_category',
    {},
...
test('position management state does not query positions without contributed query permission', async () => {
  const requests: unknown[] = [];
  const categoryContext = createContext<PositionCategory>(
    'iam.position_category',
    {},
