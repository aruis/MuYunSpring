...
test('application management state keeps existing alias stable while editing title', async () => {
  const calls: unknown[] = [];
  const context = createContext({
...
test('application management state toggles enable state and refreshes selected record', async () => {
  const calls: string[] = [];
  const context = createContext({
    disable: async (id) => {
...
test('application management state runs standard action effects before custom handlers', async () => {
...
test('application management state stays readonly after deleting last application without create permission', async () => {
  const calls: string[] = [];
  const context = createContext(
    {
      delete: async (id) => {
