...
import type { Metadata, MetadataField, ModuleMetadataRelation } from '../src/web-contracts/index.ts';
import {
  ...
  relationRoleTag,
} from '../src/views/metadataOrchestrationState.ts';
...
test('main metadata draft requires alias and title after normalization', () => {
...
test('orchestratable fields keep only business-owned physical fields', () => {
...
test('field draft validation and normalization trim identity fields', () => {
...
test('entity explorer item maps title, alias and relation role tag', () => {
...
test('entity explorer item falls back to relation alias and mutes disabled metadata', () => {
...
test('field spec options skip disabled specs and unnamed values', () => {
...
test('metadata subtitle joins alias and physical table', () => {
  assert.equal(
...
test('relations reload keeps selection when it still exists', () => {
...
test('relations reload falls back to first relation when selection vanished', () => {
  const state = createMetadataOrchestrationState();
  state.handleRelationsLoaded([relation('rel-1'), relation('rel-2')]);
  state.selectRelation(relation('rel-2'));
...
test('selecting the same relation does not reload details but exits editor modes', () => {
  const state = createMetadataOrchestrationState();
  state.handleRelationsLoaded([relation('rel-1')]);
...
test('field editor requires a selected metadata and cancels back to it', () => {
  const state = createMetadataOrchestrationState();
  state.handleRelationsLoaded([relation('rel-1')]);
...
test('create main metadata editor resets draft on every open', () => {
  const state = createMetadataOrchestrationState();
...
test('focus relation moves selection after creating main metadata', () => {
  const state = createMetadataOrchestrationState();
  state.handleRelationsLoaded([relation('rel-1')]);
  state.startCreateMain();
