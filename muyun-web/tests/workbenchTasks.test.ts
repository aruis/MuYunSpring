...
test('workspace view registry restores only a declared and valid view input', () => {
...
test('workspace view descriptors have stable parameter identity and presentation', () => {
  const descriptor = createWorkspaceViewDescriptor(view, { recordId: 'user-1' });
...
test('workspace view descriptors may use a resolved main title without changing URL identity', () => {
...
test('workspace drawer descriptors are restored and dismissed as ordinary route descriptors', () => {
...
test('workspace view registry rejects duplicate view types', () => {
  assert.throws(() => createWorkspaceViewRegistry([view, view]), /重复的工作视图类型/);
});

test('workspace view promotion requires a stable, idle source host', () => {
...
test('role detail workspace identity includes its authorization scope', () => {
...
test('dismissing a role drawer restores its owner scope without retaining work-view identity', () => {
...
test('system user detail workspace has an independent stable identity', () => {
